<template>
  <el-container class="home_container">
    <el-header>
      <div class="home_title">知识管理平台</div>
    ...
    <el-container>
      <el-aside width="200px">
        <el-menu
          ...
          <template v-for="(item,index) in filterUnhidden(this.$router.options.routes)">
            <el-submenu :index="index+''" v-if="item.children.length>1" :key="index">
              <template slot="title">
                <i :class="item.iconCls"></i>
                <span>{{item.name}}</span>
              </template>
              <el-menu-item v-for="child in filterUnhidden(item.children)"  :index="child.path" :key="child.path">
                {{child.name }}
              </el-menu-item>
            </el-submenu>
            <template v-else>
